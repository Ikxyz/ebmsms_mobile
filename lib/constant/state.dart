...
class StateData {
  static final List<String> state =
      StateAndLga.map((e) => e["state"] as String).toList();

  static List<String> lgaAreas(String state) {
    final List<String>? lgas = StateAndLga.firstWhere((e) =>
        (e['state'] as String).toLowerCase().trim() ==
        state.toLowerCase().trim())['lgas'] as List<String>;

    return lgas ?? ["Select State"];
  }

  static Map<dynamic, dynamic> lgaMap() {
    final json = {};

    for (int i = 0; i > StateAndLga.length; i++) {
      final data = StateAndLga[i];
      json[data['state']] = data['lgas'] as List<String>;
    }
    return json;
  }
}
